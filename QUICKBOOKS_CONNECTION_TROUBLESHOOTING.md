...
```bash
curl 'https://serviceflow-hub-1.preview.emergentagent.com/api/quickbooks/auth/connect?user_id=test_user'
```
...
2. **Go to:** https://serviceflow-hub-1.preview.emergentagent.com/settings/quickbooks
...
3. Check if callback endpoint is accessible:
   ```bash
   curl -I https://serviceflow-hub-1.preview.emergentagent.com/api/quickbooks/auth/callback
   ```
...
```bash
# Test authorization URL generation
curl -s 'https://serviceflow-hub-1.preview.emergentagent.com/api/quickbooks/auth/connect?user_id=test' | python3 -m json.tool

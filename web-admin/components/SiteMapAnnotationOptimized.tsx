  ...
  return (
    <div className="flex flex-col h-full bg-gradient-to-br from-gray-50 to-gray-100">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex items-center gap-1 pr-3 border-r border-gray-300">
          <button onClick={handleUndo} disabled={historyStep <= 0} className="p-2 hover:bg-white rounded-lg transition-colors disabled:opacity-50 shadow-sm" title="Undo">
            <Undo className="w-4 h-4" />
          </button>
          <button onClick={handleRedo} disabled={historyStep >= history.length - 1} className="p-2 hover:bg-white rounded-lg transition-colors disabled:opacity-50 shadow-sm" title="Redo">
            <Redo className="w-4 h-4" />
          </button>
        </div>

        <div className="flex items-center gap-1 pr-3 border-r border-gray-300">
          <button onClick={() => setTool('pen')} className={`p-2 rounded-lg transition-colors shadow-sm ${tool === 'pen' ? 'bg-blue-500 text-white' : 'hover:bg-white'}`} title="Freehand">
            <Pen className="w-4 h-4" />
          </button>
          <button onClick={() => setTool('text')} className={`p-2 rounded-lg transition-colors shadow-sm ${tool === 'text' ? 'bg-blue-500 text-white' : 'hover:bg-white'}`} title="Text">
            <Type className="w-4 h-4" />
          </button>
          <button onClick={() => setTool('circle')} className={`p-2 rounded-lg transition-colors shadow-sm ${tool === 'circle' ? 'bg-blue-500 text-white' : 'hover:bg-white'}`} title="Circle">
            <CircleIcon className="w-4 h-4" />
          </button>
          <button onClick={() => setTool('rectangle')} className={`p-2 rounded-lg transition-colors shadow-sm ${tool === 'rectangle' ? 'bg-blue-500 text-white' : 'hover:bg-white'}`} title="Rectangle">
            <Square className="w-4 h-4" />
          </button>
        </div>

        <div className="flex items-center gap-1 pr-3 border-r border-gray-300">
          <button onClick={handleZoomIn} className="p-2 hover:bg-white rounded-lg transition-colors shadow-sm" title="Zoom In">
            <ZoomIn className="w-4 h-4" />
          </button>
          <span className="text-xs font-medium text-gray-600 min-w-[40px] text-center">{Math.round(zoom * 100)}%</span>
          <button onClick={handleZoomOut} className="p-2 hover:bg-white rounded-lg transition-colors shadow-sm" title="Zoom Out">
            <ZoomOut className="w-4 h-4" />
          </button>
          <button onClick={handleResetView} className="p-2 hover:bg-white rounded-lg transition-colors shadow-sm" title="Reset">
            <Maximize2 className="w-4 h-4" />
          </button>
        </div>

        <div className="flex items-center gap-1 pr-3 border-r border-gray-300">
          <button onClick={() => setShowGrid(!showGrid)} className={`p-2 rounded-lg transition-colors shadow-sm ${showGrid ? 'bg-blue-500 text-white' : 'hover:bg-white'}`} title="Grid">
            <Grid className="w-4 h-4" />
          </button>
          <label className="flex items-center gap-1 cursor-pointer p-2 hover:bg-white rounded-lg">
            <Palette className="w-4 h-4" />
            <input type="color" value={selectedColor} onChange={(e) => setSelectedColor(e.target.value)} className="w-6 h-6 cursor-pointer" />
          </label>
        </div>

        <div className="flex items-center gap-1">
          <button onClick={clearCanvas} className="p-2 hover:bg-red-50 text-red-600 rounded-lg transition-colors shadow-sm" title="Clear">
            <Trash2 className="w-4 h-4" />
          </button>
          <button onClick={exportAsPNG} className="px-3 py-1.5 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors text-sm shadow-sm">PNG</button>
          <button onClick={exportAsPDF} className="px-3 py-1.5 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors text-sm shadow-sm">PDF</button>
        </div>
      ...
      {drawingPolygon && (
        <div className="px-4 py-2 bg-blue-50 border-b border-blue-200 text-sm">
          <span className="font-medium text-blue-900">Drawing {AREA_TYPES.find(a => a.id === currentAreaType)?.label}</span>
          <span className="text-blue-700 ml-4">Click to add • Double-click to complete • Esc to cancel</span>
          {polygonPoints.length >= 2 && <span className="text-blue-600 ml-4">Points: {polygonPoints.length / 2}</span>}
        </div>
      )}
        ...
        <div className="w-72 border-l border-gray-200 overflow-y-auto bg-gray-50">
          <div className="p-4 border-b border-gray-200 bg-white">
            <div className="relative">
              ...
              <input type="text" placeholder="Search icons..." value={iconSearch} onChange={(e) => setIconSearch(e.target.value)} className="w-full pl-9 pr-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
            </div>
          </div>

          <div className="p-4">
            <h3 className="font-semibold text-gray-900 mb-3 flex items-center gap-2">
              <MapPin className="w-4 h-4" />Feature Icons<span className="ml-auto text-xs text-gray-500">{filteredIcons.length}</span>
            </h3>
            <div className="grid grid-cols-2 gap-2 mb-6">
              {filteredIcons.map((feature) => {
                const Icon = feature.icon;
                return (
                  <button key={feature.id} onClick={() => addFeatureIcon(feature.id)} className="flex flex-col items-center gap-1 p-3 border-2 border-gray-200 rounded-lg hover:border-gray-400 hover:shadow-md transition-all bg-white" style={{ borderColor: feature.color + '40' }}>
                    <Icon className="w-5 h-5" style={{ color: feature.color }} />
            ...
            <h3 className="font-semibold text-gray-900 mb-3 flex items-center gap-2">
              <Layers className="w-4 h-4" />Area Types
            </h3>
            <div className="space-y-2 mb-6">
              {AREA_TYPES.map((area) => (
                <button key={area.id} onClick={() => addAreaMarker(area.id)} className={`w-full flex items-center gap-3 p-3 border-2 rounded-lg hover:border-gray-400 hover:shadow-md transition-all bg-white ${drawingPolygon && currentAreaType === area.id ? 'border-blue-500 bg-blue-50' : 'border-gray-200'}`}>
                  <div className="w-6 h-6 rounded border-2 border-white shadow-sm" style={{ backgroundColor: area.color }} />
            ...
            <div className="p-4 bg-white rounded-lg border-2 border-gray-200 shadow-sm">
              <h4 className="text-sm font-semibold text-gray-900 mb-3 flex items-center gap-2">
                <CheckCircle className="w-4 h-4" />Map Info
              </h4>

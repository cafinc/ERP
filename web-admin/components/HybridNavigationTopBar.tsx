...
import ModernHeaderWithNav from './ModernHeaderWithNav';
import {
  LayoutDashboard,
  Users,
  FileText,
...
export default function HybridNavigationTopBar({ children }: { children: React.ReactNode }) {
  ...
  const menuItems: MenuItem[] = [
    ...
    },
    {
      ...
      submenu: [
        ...
        { label: 'AP Dashboard', href: '/finance/ap-dashboard', icon: LayoutDashboard },
        { label: 'Bills', href: '/finance/bills', icon: FileText },
  ...
  const handleMenuClick = (item: MenuItem) => {
    if (item.submenu) {
      // If it has submenu, just toggle the submenu - don't navigate
      setExpandedMenu(expandedMenu === item.label ? null : item.label);
    } else if (item.href) {
      // Single page without submenu - navigate directly

...
```bash
# Check connection status
curl -X GET "https://mapbuilder-3.preview.emergentagent.com/api/quickbooks/connection/status?user_id=YOUR_USER_ID"
```
...
```bash
curl -X POST "https://mapbuilder-3.preview.emergentagent.com/api/quickbooks/invoices?user_id=YOUR_USER_ID" \
  -H "Content-Type: application/json" \
  -d '{
    "CustomerRef": {

...
```bash
# Check connection status
curl -X GET "https://glass-admin-dash.preview.emergentagent.com/api/quickbooks/connection/status?user_id=YOUR_USER_ID"
```

### Step 2: Test Customer Creation

```bash
curl -X POST "https://glass-admin-dash.preview.emergentagent.com/api/quickbooks/customers?user_id=YOUR_USER_ID" \
...
```bash
curl -X POST "https://glass-admin-dash.preview.emergentagent.com/api/quickbooks/invoices?user_id=YOUR_USER_ID" \
  -H "Content-Type: application/json" \
  -d '{
    "CustomerRef": {
